...
export class ReportsController {
  ...
  async createPenyambungan(
    ...
    return this.reportsService.createPenyambungan(
      createPenyambunganDto,
      files,
      userId,
    );
  }
  ...
  getPerformanceStats(@Query() filterDto: PerformanceFilterDto) {
    return this.reportSummaryService.getPerformanceStats(
      filterDto.period || PeriodType.MONTHLY,
    );
  }
  ...
  findAll(
    ...
    return this.reportsService.FindActiveReport(
      paginationQuery,
      userId,
      userRole,
    );
  ...
  findPenyambungan(
    ...
    const userId = req.user?.id;
    const userRole = req.user?.role;
    return this.reportsService.findPenyambunganReports(
      paginationQuery,
      userId,
      userRole,
    );
  ...
  async exportExcel(
    ...
    const buffer =
      await this.reportExportService.exportReportsToExcel(filterDto);
  ...
  findYantekHistoryForPetugas(
    ...
    if (!userFullname) {
      throw new BadRequestException(
        'User name is required for PETUGAS_YANTEK history.',
      );
    }
    return this.reportsService.findYantekHistoryForPetugas(
      paginationQuery,
      userFullname,
    );
  }
  ...
  findPenyambunganHistoryForPetugas(
    ...
    if (!userId) {
      throw new BadRequestException(
        'User ID is required for PETUGAS_PENYAMBUNGAN',
      );
    }
    return this.reportsService.findPenyambunganHistoryForPetugas(
      paginationQuery,
      userId,
    );
  }
